    ...
    pub fn new(raw: impl Into<String>) -> Result<Self, DomainError> {
        ...
        if trimmed.len() < 10 {
            return Err(DomainError::ValidationError(
                format!("Transaction hash too short: '{}'", s)
            ));
        }
        if !trimmed.chars().all(|c| c.is_ascii_hexdigit()) {
            return Err(DomainError::ValidationError(
                format!("Transaction hash contains non-hex characters: '{}'", s)
            ));
        }
        Ok(Self(s))
    }
...
impl WalletAddress {
    pub fn new(raw: impl Into<String>) -> Result<Self, DomainError> {
        let s = raw.into();
        if s.trim().is_empty() {
            return Err(DomainError::ValidationError("Wallet address cannot be empty".to_string()));
        }
        if s.len() < 26 {
            return Err(DomainError::ValidationError(
                format!("Wallet address too short: '{}'", s)
            ));
        }
        Ok(Self(s))
    }
...
impl IdempotencyKey {
    ...
    pub fn generate() -> Self { Self(uuid::Uuid::new_v4()) }
    pub fn value(&self) -> uuid::Uuid { self.0 }
}

...
impl EvmGateway {
    pub fn new_ethereum(api_key: String, wallets: HashMap<String, String>) -> Self {
        Self::build(api_key, "https://api.etherscan.io/api".to_string(), wallets, 12)
    }

    pub fn new_bsc(api_key: String, wallets: HashMap<String, String>) -> Self {
        Self::build(api_key, "https://api.bscscan.com/api".to_string(), wallets, 15)
    }

    fn build(api_key: String, base_url: String, wallets: HashMap<String, String>, min_confirmations: u32) -> Self {
        let client = Client::builder()
            .timeout(Duration::from_secs(10))
            .build()
            .expect("Failed to build HTTP client");
        Self { client, api_key, base_url, wallets, min_confirmations }
    }
    ...
    async fn current_block(&self) -> Result<u64, InfraError> {
        let url = format!("{}?module=proxy&action=eth_blockNumber&apikey={}", self.base_url, self.api_key);
...
impl BlockchainGateway for EvmGateway {
    ...
    async fn verify_transaction(
        ...
        if self.api_key.trim().is_empty() {
            return Err(DomainError::InternalError("Missing blockchain API key".to_string()));
        }
        ...
        let tx_block = result["blockNumber"].as_str()

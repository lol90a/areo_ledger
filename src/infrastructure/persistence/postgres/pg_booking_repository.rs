...
impl PgBookingRepository {
    ...
    fn uuid_from_row(row: &tokio_postgres::Row, idx: usize, field: &str) -> Result<Uuid, DomainError> {
        let raw: String = row.try_get(idx)
            .map_err(|e| DomainError::InternalError(format!("error retrieving {}: {}", field, e)))?;
        Uuid::from_str(raw.trim()).map_err(|e| {
    ...
    fn row_to_booking(row: &tokio_postgres::Row) -> Result<Booking, DomainError> {
        let mk = |cents: i64| Money::from_cents(cents, FiatCurrency::Usd)
        ...
        Ok(Booking::from_db(
            ...
            Self::uuid_from_row(row, 2, "booking.flight_id")?,
...
impl BookingRepository for PgBookingRepository {
    async fn find_by_id(&self, id: &Uuid) -> Result<Booking, DomainError> {
        ...
        let row = client.query_one(
            ...
            &[id],
        ).await.map_err(|_| DomainError::NotFound(format!("Booking {}", id)))?;

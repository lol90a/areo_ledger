...
impl<T: Serialize> ApiResponse<T> {
    pub fn ok(data: T) -> Self {
        Self { success: true, data: Some(data), error: None, request_id: Uuid::new_v4() }
    }
}

impl ApiResponse<()> {
    pub fn err(code: &'static str, message: String) -> Self {
        Self { success: false, data: None, error: Some(ApiError { code, message }), request_id: Uuid::new_v4() }
    }
}
...
pub fn domain_error_to_response(err: DomainError) -> HttpResponse {
    match err {
        ...
        DomainError::Unauthorized =>
            HttpResponse::Unauthorized().json(ApiResponse::<()>::err("UNAUTHORIZED", "Authentication required".to_string())),
        ...
        DomainError::InternalError(msg) => {
            ...
            HttpResponse::InternalServerError().json(ApiResponse::<()>::err("INTERNAL_ERROR", "An unexpected error occurred".to_string()))
        }
    }
}
